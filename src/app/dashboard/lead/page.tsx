...
import { Plus, Target, DollarSign, TrendingUp, Users, Save } from 'lucide-react';
    ...
    <PageContainer title="Lead & Pipeline" description="Gestione opportunità commerciali" actions={
      ...
      <Tabs defaultValue="pipeline">
                ...
                <Card key={fase} className={`border-t-4 ${faseBorderColor[fase]}`}>
                  <CardHeader className="p-3 pb-2">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant="secondary" className="text-[10px]">{faseLeads.length}</Badge>
                    </div>
                    <p className="text-xs text-muted-foreground">{formatCurrency(faseLeads.reduce((s, l) => s + l.valore, 0))}</p>
                  </CardHeader>
                  <CardContent className="p-3 pt-0 space-y-2">
                    {faseLeads.map((lead) => (
                      <div key={lead.id} className="rounded-lg border p-2.5 bg-background hover:shadow-sm transition-shadow">
                        ...
                        <div className="flex items-center justify-between mt-2">
                          ...
                          <span className="text-[10px] text-muted-foreground">{lead.probabilita}%</span>
                        </div>
                        <p className="text-[10px] text-muted-foreground mt-1">{lead.assegnatoNome}</p>
                      </div>
                    ))}
        ...
        <TabsContent value="lista">
          <Card><CardContent className="p-0"><Table><TableHeader><TableRow><TableHead>Azienda</TableHead><TableHead className="hidden md:table-cell">Referente</TableHead><TableHead className="hidden lg:table-cell">Fonte</TableHead><TableHead>Fase</TableHead><TableHead className="text-right">Valore</TableHead><TableHead className="text-center hidden md:table-cell">Prob.</TableHead><TableHead className="hidden lg:table-cell">Assegnato</TableHead></TableRow></TableHeader>
